<?php namespace Eq3w\Forum\Classes;
...
class TopicTracker
{
    ...
    public function getNewTopics($member)
    {
        ...
        $results = Db::table('eq3w_forum_topics')->select([
                'eq3w_forum_topics.id',
                'eq3w_forum_topics.channel_id',
                'eq3w_forum_topics.last_post_at'
            ])
            ->join('eq3w_forum_channels', 'eq3w_forum_channels.id', '=', 'eq3w_forum_topics.channel_id')
            ->where('eq3w_forum_topics.last_post_at', '>', $lastLogin)
            ->get()
        ;
